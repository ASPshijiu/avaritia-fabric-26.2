...
本矩阵记录实现范围与自动化行为测试基线。2026-07-21 复核发现，原表把 GameTest、客户端资源加载和专服烟雾测试统一写成“已验证”，不能据此声称每个物品都经过真实客户端人工操作。

真实游戏内逐项操作的权威记录改为 [INGAME_TEST_REPORT.md](INGAME_TEST_REPORT.md)。只有该报告完成对应物品的实际合成/产出、使用、功能观察、保存重进及适用的多人验证后，才可称为“游戏内实测通过”。

本表状态值：`未开始`、`实现中`、`已验证`、`不适用`。其中“已验证/通过”仅表示现有自动化断言或运行时烟雾证据通过，不等同于人工游戏内验收。
...
仅在对应 `c:ingots/*` tag 非空时启用。生产数据包在标签为空时保持禁用；测试数据包为每个标签提供独立输入。13 种变体除 GameTest 外，均已由真实客户端逐项完成 GUI 压制、取出、显示、保存重进。专用服务器上的奇点定义依赖 `sync_singularities` S2C 同步包在加入/重载时推送；双客户端同时在线场景的验证边界见 KNOWN_ISSUES。
...
- 功能矩阵共 114 行，状态均为 `已验证`；其中通用 `singularity` 是 27 种组件变体共用的注册物品，所以游戏内报告按具体可操作对象记录为 113 项，113 项状态均为 `通过`。
- `eternal_singularity` 与 `infinity_catalyst` 依赖奇点数据组件区分变体，已明确声明为不参与原版自动填充的动态特殊配方；90 项 GameTest、单人客户端和专用服务器生产数据启动均确认不再出现空材料放置警告，实际 9×9 合成行为保持通过。
- 生产数据启动加载 14 个内置奇点；13 个条件式科技标签非空时加载 27 个。测试数据包已从单人及专服存档中正常禁用并移除，两个存档最终均保持和平模式并完成保存。
- 最终未验证对象：0。
