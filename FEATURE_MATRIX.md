...
仅在对应 `c:ingots/*` tag 非空时启用。生产数据包在标签为空时保持禁用；测试数据包为每个标签提供独立输入。13 种变体除 GameTest 外，均已由真实客户端逐项完成 GUI 压制、取出、显示、保存重进，并在专用服务器双客户端中完成同步和同名重连验证。
...
- `eternal_singularity` 与 `infinity_catalyst` 依赖奇点数据组件区分变体，已明确声明为不参与原版自动填充的动态特殊配方；89 项 GameTest、单人客户端和专用服务器生产数据启动均确认不再出现空材料放置警告，实际 9×9 合成行为保持通过。
